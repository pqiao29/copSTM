...
double score_main_cpp(const vector<int>& pair_ind, const int& par_pos, const double& rho,
  ...
  vector<double> bd1{a1, b1}, bd2{a2, b2};
 ...
 */
double score_rho_cpp(const vector<int>& pair_ind, double rho, 
                     const vector<double>& lower_bd, const vector<double>& upper_bd, 
                     const double& pi){
  int i = pair_ind[0], j = pair_ind[1];
  double a1 = lower_bd[i], a2 = lower_bd[j], b1 = upper_bd[i], b2 = upper_bd[j];
